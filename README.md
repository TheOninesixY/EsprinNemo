...
网络功能仅有两项，且均需手动配置后才可能发起请求：可选的自建同步（配套 Python 服务端，以操作日志在设备之间同步，删除记录不会被其他设备重新生成）与内置的 AI 助手。两者均未配置时，应用不发起任何外部请求。
...
- **禁用标签页**：仅现代布局下出现的开关（写入 `config.json` 的 `tabsDisabled`）。开启后标签栏不参与排版，编辑器顶栏及以下整列上移，内容直达窗口上沿；标签状态不会丢失，关闭开关即恢复。切回经典布局时该选项失效，偏好保留
...
- **拖动窗口**：由编辑器顶栏空闲段、工作区标签栏空白、笔记列表顶部搜索栏行、设置页标题区与分类侧边栏、空状态工作区、AI 面板顶栏上方留白，以及侧边栏「新建」周边留白承担
...
- **自动同步**：可选「每 5 秒 / 每 1 分钟 / 每 5 分钟 / 每次启动应用时 / 自定义（自行填写秒数或分钟数）」，到点自动执行「拉取 → 重放 → 推送」
...
| 文件 | 内容 |
| --- | --- |
| `journal.log` | 全部数据（append-only 操作日志） |
...
```bash
# 服务端：接口、幂等、越界路径、重启后重建索引、管理后台（设密码 / 登录 / 令牌增删改查 / 设备绑定）
python EsprinServer.py --selftest

# 客户端同步逻辑：删除不复活、远端更新覆盖本地、本地更新不被旧操作覆盖等
...
│       ├── styles/         # tokens / base / sidebar / editor / overlays / dropdown（自绘下拉菜单）/ settings / ai / sync（自建同步面板）/ alom（Alom 风格）/ radius（圆角尺度）/ mode（界面布局）/ motion（动效：进场动画与换肤交叉淡入）
│       └── scripts/        # state / storage / markdown / ui / dropdown / theme_color / mode / notes / scratchpad / tray / auto_launch / sync_server / editor / ai / ai_chats / ai_files / ai_agent / update / render …
