...
- **小本本**：标题栏一键唤出屏幕右下角的便利贴小窗口，始终置顶，标题栏只有「选择笔记 / 保存 / 最小化 / 关闭」；自带标题与正文输入，自动保存，`Ctrl + S` 保存。未关联笔记时内容属于小本本自己（数据目录下的 `scratchpad.json`），点保存即在后台新建一篇笔记并自动关联；点选择可搜索并打开已有笔记，之后小本本里编辑的就是那篇笔记。小本本开着时关闭主窗口不会退出应用：主窗口只是被收起，应用继续在小本本里运行，此间再次打开应用会把主窗口唤回来（不会另起一个进程），直到小本本也被关闭应用才退出

  ![小本本截图](readme/scratchpad.png)

- **系统托盘**：默认在任务栏通知区域显示托盘图标，可在「设置 → 系统与托盘」中关闭。右键图标即可快捷打开主窗口、小本本、新建笔记 / 新建待办、进入设置或退出；左键单击直接把主窗口唤回前台
- **驻留后台**：托盘图标在时关闭窗口只是把界面收起来（应用继续在托盘里运行），要完全退出请用托盘菜单里的「退出」；关闭托盘后恢复原有行为：关闭主窗口即退出应用，只有小本本还开着时才继续驻留
- **开机自启**：可在「设置 → 系统与托盘」中开启，登录系统后自动启动（写入系统的登录启动项：Windows 为注册表 Run 项，macOS 为登录项，Linux 为 `~/.config/autostart` 下的 `.desktop`）；默认关闭，开发运行（`bun start`）不提供该开关
...
- **更新提醒可关断**：更新弹窗上带一个「永不提醒」勾选框，勾选后关闭窗口（“稍后”或右上角关闭）即把配置里的「自动检查并下载更新」改成关闭，以后不再自动检查与下载；需要时在设置里重新打开即可
...
- 同时产出便携版 `dist/EsprinNemo.Portable.x.y.z.exe`：每次启动解压到临时目录运行，
  **数据与配置都放在便携版 exe 所在的目录**，不写 `%APPDATA%`；便携版没有稳定的可升级目标，
  因此不做更新检查（见下文「数据存储」）
...
```
data/
├── config.json     # 偏好设置：主题、主题色、应用名颜色、字体、拼写检查、废纸篓保留天数、自动更新开关、开机自启开关、托盘图标开关、自定义文件夹、AI 接口配置、当前选中的对话等
...
│       └── scripts/        # state / storage / markdown / ui / theme_color / notes / scratchpad / tray / auto_launch / editor / ai / ai_chats / ai_files / ai_agent / update / render …
